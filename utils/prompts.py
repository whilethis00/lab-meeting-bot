...
SUMMARIZER_PROMPT = """다음은 구조화된 회의 데이터입니다. {lang_instruction}
...
SMART_SPEAKER_ASSIGN_PROMPT = """회의록의 화자 라벨과 실제 참여자 이름을 매핑하세요.

각 화자의 발언 샘플을 보고, 제공된 이름 목록과 가장 잘 맞는 사람을 추론하세요.

추론 힌트:
- 질문을 많이 하고 방향을 제시하는 사람 → 교수님/지도교수
- 연구 결과를 발표하거나 진행상황을 보고하는 사람 → 대학원생
- 짧게 대답하거나 발언이 적은 사람 → 해당 역할 고려
- 이름이 "나"로 제공된 경우 → 주로 발표 또는 질의응답을 주도하는 화자

화자별 발언 샘플:
{speaker_samples}

참여자 이름 목록 (순서는 힌트일 뿐, 순서대로 매핑하지 말 것):
{names}

JSON 배열로만 응답:
[{{"label": "화자 라벨", "name": "실제 이름", "reason": "추론 근거 한 줄"}}]"""
